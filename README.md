...
- **SQL-хранилище (PostgreSQL)** — сообщения, баны и муты переживают перезапуски сервера.
...
GUI-клиент чата, запускается через executor (инжектор) Roblox. Есть **две версии**:

| Файл | Кому | Особенности |
|------|------|-------------|
| `client/RussChat.lua` | **Публичная** — раздаёшь людям | обычный чат, `ADMIN_SECRET` пустой, без bypass |
| `client/RussChat_Admin.lua` | **Только владельцу** — НЕ раздавать | bypass (без кулдауна/мута/фильтра); нужно вписать свой `ADMIN_SECRET` |

Обе версии идентичны по коду и интерфейсу — отличается только блок `CONFIG`.

- Красивое перетаскиваемое окно: список сообщений, поле ввода, кнопки **Отправить / Свернуть / Закрыть**.
- Логин на сервере (ник из `LocalPlayer.Name`), опрос `/chat`, отправка сообщений.
- Сам обрабатывает протухший токен (401 → перелогин) и rate-limit (429).

Настройка:
- `CONFIG.SERVER_URL` — адрес твоего сервера на Render.
- `CONFIG.CLIENT_KEY` — тот же ключ, что в env-переменной `CLIENT_KEY` на сервере (уже вписан в обе версии).
- `CONFIG.ADMIN_SECRET` — **только** в `RussChat_Admin.lua`: вставь свой секрет с Render. В публичной версии — пусто.

> ⚠️ Никогда не публикуй `RussChat_Admin.lua` с заполненным `ADMIN_SECRET` — это даст bypass любому.
...
(`/chat`), и в клиенте (владельцу не включается локальный кулдаун).
